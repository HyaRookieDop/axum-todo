...
async fn query<T, C>(client: &C, sql: &str, params: &[&(dyn ToSql + Sync)]) -> Result<Vec<T>>
where
    C: GenericClient,
...
async fn query_one<T, C>(client: &C, sql: &str, params: &[&(dyn ToSql + Sync)]) -> Result<T>
where
    C: GenericClient,
    T: FromTokioPostgresRow,
{
...
async fn excute<C: GenericClient>(
    client: &C,
    sql: &str,
